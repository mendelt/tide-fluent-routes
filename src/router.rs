...
pub trait Router<State: Clone + Send + Sync + 'static> {
    ...
    fn route(&mut self, route: impl Fn(RouteBuilder<State>) -> RouteBuilder<State>) {
        todo!()
    }
}
...
impl<State: Clone + Send + Sync + 'static> RouteBuilder<State> {
    pub fn at(self, path: &str, subroute: impl Fn(RouteBuilder<State>) -> RouteBuilder<State>) -> RouteBuilder<State> {
        todo!()
    }

    pub fn method(self, path: &str, method: Method, endpoint: impl Endpoint<State>) -> RouteBuilder<State> {
        todo!()
    }
}
